...
console.log("Challenge 1 .forEach()");
...
console.log("Challenge 4 .reduce()");
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 -- The shirts have been ordered but now we need to prepare them to be given out. Create a list that shows last name and shirt size.

let nameSize = [];

runners.forEach(function(item){
  return nameSize.push(`${item.last_name} ${item.shirt_size}`);
});
console.log("Challenge 5.1");
console.log(nameSize);
// Problem 2 - the company names would look better if they were all capitalized.

let company = [];

company = runners.map(function(item){
...
// Problem 3  The company Skinix won a prize so everyone who is from that company gets a reward.

let winners = [];

winners = runners.filter(function(item){
